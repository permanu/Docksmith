...
func mustPlanPython(t *testing.T, fw *Framework) *BuildPlan {
	t.Helper()
...
func TestPlanPython_DefaultVersion(t *testing.T) {
	fw := &Framework{Name: "flask", Port: 5000, StartCommand: "gunicorn app:app"}
...
func TestPlanPython_VenvCopied(t *testing.T) {
	...
	for _, step := range runtime.Steps {
		if step.Type == StepCopyFrom && step.CopyFrom != nil && step.CopyFrom.Src == "/app/.venv" {
			return
		}
	}
...
func TestPlanPython_PathEnvSet(t *testing.T) {
	runtime := mustPlanPython(t, djangoFramework()).Stages[1]
	for _, step := range runtime.Steps {
		if step.Type == StepEnv && len(step.Args) >= 2 && step.Args[0] == "PATH" &&
...
func TestPlanPython_GunicornBindFixup(t *testing.T) {
	fw := &Framework{Name: "django", Port: 8000, StartCommand: "gunicorn myapp.wsgi:application"}
	runtime := mustPlanPython(t, fw).Stages[1]
	for _, step := range runtime.Steps {
...
func TestPlanPython_GunicornBindNotDuplicated(t *testing.T) {
	fw := &Framework{Name: "django", Port: 8000, StartCommand: "gunicorn myapp.wsgi:application --bind 0.0.0.0:8000"}
	runtime := mustPlanPython(t, fw).Stages[1]
	for _, step := range runtime.Steps {
		if step.Type == StepCmd {
			cmd := strings.Join(step.Args, " ")
