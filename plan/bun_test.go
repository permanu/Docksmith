...
func makeElysiaFramework() *core.Framework {
...
func mustPlanBun(t *testing.T, fw *core.Framework) *core.BuildPlan {
	t.Helper()
	plan, err := planBun(fw)
	if err != nil {
...
func TestPlanBun_NoBuildCommand_SkipsBuildRun(t *testing.T) {
	...
	for _, s := range build.Steps {
		if s.Type == core.StepRun && len(s.Args) == 1 && s.Args[0] == "" {
...
func TestPlanBun_Runtime_HasBunUser(t *testing.T) {
	...
	runtime := plan.Stages[1]
	for _, s := range runtime.Steps {
		if s.Type == core.StepUser && s.Args[0] == "bun" {
			return
		}
	}
