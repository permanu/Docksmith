...
func TestFrameworkDefaults(t *testing.T) {
	known := []struct {
		...
		{"django", "pip install -r requirements.txt", "gunicorn config.wsgi:application --bind 0.0.0.0:${PORT:-8000} --workers ${WEB_CONCURRENCY:-2} --threads 2"},
		{"fastapi", "pip install -r requirements.txt", "gunicorn main:app --bind 0.0.0.0:${PORT:-8000} --workers ${WEB_CONCURRENCY:-2} -k uvicorn.workers.UvicornWorker"},
		{"flask", "pip install -r requirements.txt", "gunicorn app:app --bind 0.0.0.0:${PORT:-8000} --workers ${WEB_CONCURRENCY:-2} --threads 2"},
