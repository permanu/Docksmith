...
func springBootFramework() *core.Framework {
...
func gradleFramework() *core.Framework {
	return &core.Framework{
		Name:         "gradle",
...
func mustPlanJava(t *testing.T, fw *core.Framework) *core.BuildPlan {
	t.Helper()
...
func TestPlanJava_Maven_BuilderHasCacheMount(t *testing.T) {
	...
	found := false
	for _, step := range builder.Steps {
		if step.Type == core.StepRun && step.CacheMount != nil && step.CacheMount.Target == "/root/.m2" {
			found = true
		}
	}
	if !found {
...
func TestPlanJava_Maven_RuntimeCopiesJAR(t *testing.T) {
	...
	for _, step := range runtime.Steps {
		if step.Type == core.StepCopyFrom && step.CopyFrom != nil {
...
func TestPlanJava_Gradle_CacheMount(t *testing.T) {
	...
	found := false
	for _, step := range builder.Steps {
		if step.Type == core.StepRun && step.CacheMount != nil && step.CacheMount.Target == "/root/.gradle" {
			found = true
		}
	}
	if !found {
...
func TestPlanJava_DefaultJavaVersion(t *testing.T) {
	fw := &core.Framework{Name: "maven", BuildCommand: "mvn package -DskipTests", Port: 8080}
