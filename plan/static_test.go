...
func staticFramework() *core.Framework {
	return &core.Framework{
		Name: "static",
	}
}

func mustPlanStatic(t *testing.T, fw *core.Framework) *core.BuildPlan {
	t.Helper()
...
func TestPlanStatic_CustomOutputDir(t *testing.T) {
	fw := &core.Framework{Name: "static", OutputDir: "dist"}
...
func TestPlanStatic_DefaultOutputDir(t *testing.T) {
	...
	stage := plan.Stages[0]
	for _, step := range stage.Steps {
		if step.Type == core.StepCopy && len(step.Args) > 0 {
			if step.Args[0] != "." {
