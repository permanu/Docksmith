...
func makeNextJSFramework(pm string) *core.Framework {
	return &core.Framework{
		Name:           "nextjs",
		BuildCommand:   detect.PMRunBuild(pm),
		StartCommand:   detect.PMRunStart(pm),
		Port:           3000,
		OutputDir:      ".next",
		NodeVersion:    "22",
		PackageManager: pm,
...
func mustPlanNode(t *testing.T, fw *core.Framework) *core.BuildPlan {
	t.Helper()
...
func TestPlanNode_CacheMount_ByPM(t *testing.T) {
	...
	for _, tc := range cases {
		...
		deps := plan.Stages[0]
		var found *core.CacheMount
		for _, s := range deps.Steps {
			if s.Type == core.StepRun && s.CacheMount != nil {
				found = s.CacheMount
				break
			}
		}
...
func hasCorepackEnable(plan *core.BuildPlan) bool {
	for _, step := range plan.Stages[0].Steps {
		if step.Type == core.StepRun && len(step.Args) == 1 && step.Args[0] == "corepack enable" {
			return true
		}
	}
	return false
...
func makeNodeFramework(pm, nodeVersion string) *core.Framework {
	return &core.Framework{
		Name:           "nextjs",
		BuildCommand:   detect.PMRunBuild(pm),
		StartCommand:   detect.PMRunStart(pm),
...
func TestPlanNode_Corepack_StepOrder(t *testing.T) {
	...
	for i, s := range steps {
		switch {
		case s.Type == core.StepWorkdir:
			workdirIdx = i
		case s.Type == core.StepRun && len(s.Args) == 1 && s.Args[0] == "corepack enable":
			corepackIdx = i
		case s.Type == core.StepCopy && copyIdx == -1:
			copyIdx = i
		}
	}
...
func TestNodeVersionAtLeast(t *testing.T) {
	...
	for _, tc := range cases {
		got := detect.NodeVersionAtLeast(tc.ver, tc.min)
		if got != tc.want {
			t.Errorf("detect.NodeVersionAtLeast(%q, %d) = %v, want %v", tc.ver, tc.min, got, tc.want)
		}
	}
}
...
func TestPlanNode_ServerRuntime_HasNodeUser(t *testing.T) {
	...
	runtime := plan.Stages[len(plan.Stages)-1]
	for _, s := range runtime.Steps {
		if s.Type == core.StepUser && s.Args[0] == "node" {
			return
		}
	}
...
func TestPlanNode_StaticRuntime_HasNginxCacheDirs(t *testing.T) {
	...
	found := false
	for _, s := range runtime.Steps {
		if s.Type == core.StepRun && len(s.Args) > 0 &&
...
func TestPlanNode_StaticRuntime_NginxCacheDirsBeforeUser(t *testing.T) {
	...
	for i, s := range runtime.Steps {
		if s.Type == core.StepRun && len(s.Args) > 0 && strings.Contains(s.Args[0], "/var/cache/nginx") {
			cacheIdx = i
		}
...
func TestPlanNode_StaticRuntime_HasNginxUser(t *testing.T) {
	...
	runtime := plan.Stages[len(plan.Stages)-1]
	for _, s := range runtime.Steps {
		if s.Type == core.StepUser && s.Args[0] == "nginx" {
			return
		}
	}
...
func TestPlanNode_StaticRuntime_CopyUsesAbsolutePath(t *testing.T) {
	...
	runtime := plan.Stages[len(plan.Stages)-1]
	for _, s := range runtime.Steps {
		if s.Type == core.StepCopyFrom && s.CopyFrom != nil && s.CopyFrom.Stage == "build" {
