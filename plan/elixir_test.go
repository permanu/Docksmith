...
func phoenixFramework() *core.Framework {
...
func mustPlanElixir(t *testing.T, fw *core.Framework) *core.BuildPlan {
	t.Helper()
...
func TestPlanElixir_BuilderSetsMixEnvProd(t *testing.T) {
	...
	for _, step := range builder.Steps {
		if step.Type == core.StepEnv && len(step.Args) >= 2 &&
...
func TestPlanElixir_RuntimeSetsPortEnv(t *testing.T) {
	...
	found := false
	for _, step := range runtime.Steps {
		if step.Type == core.StepEnv && len(step.Args) >= 1 && step.Args[0] == "PORT" {
			found = true
		}
	}
	if !found {
...
func TestPlanElixir_DefaultPort(t *testing.T) {
	fw := &core.Framework{Name: "elixir-phoenix", BuildCommand: "mix compile", StartCommand: "mix phx.server"}
