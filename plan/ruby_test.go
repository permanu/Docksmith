...
func railsFramework() *core.Framework {
...
func sinatraFramework() *core.Framework {
	return &core.Framework{
		Name:         "sinatra",
		BuildCommand: "bundle install",
...
func mustPlanRuby(t *testing.T, fw *core.Framework) *core.BuildPlan {
	t.Helper()
...
func TestPlanRuby_BuilderNoCacheMountOnBundleDir(t *testing.T) {
	...
	builder := plan.Stages[0]
	for _, step := range builder.Steps {
		if step.Type == core.StepRun && step.CacheMount != nil && step.CacheMount.Target == "/usr/local/bundle" {
...
func TestPlanRuby_DefaultPort(t *testing.T) {
	fw := &core.Framework{Name: "rails", BuildCommand: "bundle install", StartCommand: "rails server"}
...
func TestPlanRuby_Runtime_HasBundleBinInPath(t *testing.T) {
	...
	for _, s := range runtime.Steps {
		if s.Type == core.StepEnv && s.Args[0] == "PATH" {
