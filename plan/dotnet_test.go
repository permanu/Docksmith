...
func aspnetFramework() *core.Framework {
...
func dotnetWorkerFramework() *core.Framework {
	return &core.Framework{
		Name:          "dotnet-worker",
		BuildCommand:  "dotnet publish -c Release -o /app/publish",
...
func mustPlanDotnet(t *testing.T, fw *core.Framework) *core.BuildPlan {
	t.Helper()
...
func TestPlanDotnet_AspNet_RuntimeHasEntrypoint(t *testing.T) {
	...
	found := false
	for _, step := range runtime.Steps {
		if step.Type == core.StepEntrypoint {
			for _, arg := range step.Args {
...
func TestPlanDotnet_DefaultDotnetVersion(t *testing.T) {
	fw := &core.Framework{Name: "aspnet-core", Port: 8080, StartCommand: "dotnet /app/publish/App.dll"}
...
func TestPlanDotnet_Web_HasAppUser(t *testing.T) {
	...
	for _, s := range runtime.Steps {
		if s.Type == core.StepUser && s.Args[0] == "app" {
			return
		}
	}
