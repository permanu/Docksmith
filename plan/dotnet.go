...
func planDotnet(fw *core.Framework) (*core.BuildPlan, error) {
	...
	builder := core.Stage{
		...
		From: sdkImage,
		Steps: []core.Step{
			{Type: core.StepWorkdir, Args: []string{"/src"}},
			{Type: core.StepCopy, Args: []string{"*.csproj", "./"}},
			{
				Type:       core.StepRun,
				Args:       []string{"dotnet restore"},
				CacheMount: &core.CacheMount{Target: "/root/.nuget/packages"},
			},
			{Type: core.StepCopy, Args: []string{".", "."}},
			{Type: core.StepRun, Args: []string{"dotnet publish -c Release -o /app/publish"}},
		},
	}
	...
	runtime := core.Stage{
		...
		Steps: []core.Step{
			{Type: core.StepWorkdir, Args: []string{"/app"}},
			{
				...
				CopyFrom: &core.CopyFrom{Stage: "builder", Src: "/app/publish", Dst: "."},
				Link:     true,
			},
		},
	}
	...
	if isWorker {
		runtime.Steps = append(runtime.Steps,
			core.Step{Type: core.StepEntrypoint, Args: []string{"dotnet", projectName + ".dll"}},
		)
	...
	runtime.Steps = append(runtime.Steps,
		core.Step{Type: core.StepEnv, Args: []string{"ASPNETCORE_URLS", "http://+:8080"}},
		core.Step{Type: core.StepExpose, Args: []string{"8080"}},
		core.Step{Type: core.StepEntrypoint, Args: []string{"dotnet", projectName + ".dll"}},
	)
